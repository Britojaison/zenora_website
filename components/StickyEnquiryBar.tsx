...
export default function StickyEnquiryBar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (pathname === "/") {
        // On home page, wait until scrolled down past the hero section (e.g. 200px)
        setIsVisible(window.scrollY > 200);
      } else {
        // On inner pages, show immediately
        setIsVisible(true);
      }
    };
  ...
  // Parse dynamic SRD from URL parameter or UTM parameters on mount
  useEffect(() => {
      ...
      <AnimatePresence>
        {isVisible && isMobile && (
          <motion.div
            ...
            exit={{ y: 100, opacity: 0 }}
            transition={{ type: "spring", damping: 30, stiffness: 200 }}
            className="fixed bottom-0 left-0 right-0 bg-[#28362b] border-t border-[#e1b258]/30 shadow-2xl z-40 p-3 grid grid-cols-3 gap-3 font-body"
          >
            {/* Call Button */}
            <a
              href={`tel:${phoneNumber}`}
